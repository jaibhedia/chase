...
export function initializeGame(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
  ...
  const humanPlayer: Player = {
    id: 'player',
    x: Math.random() * (selectedMap.width - 200) + 100,
    y: Math.random() * (selectedMap.height - 200) + 100,
  ...
  for (let i = 0; i < numBots; i++) {
    const botChar = characters[i % characters.length];
    players.push({
      id: `bot-${i}`,
      x: Math.random() * (selectedMap.width - 200) + 100,
      y: Math.random() * (selectedMap.height - 200) + 100,
...
function createMapObjects(mapId: string, width: number, height: number): GameObject[] {
  ...
  if (mapId === 'map-1') {
    objects.push(
      { id: 'sofa', x: 150, y: 200, width: 120, height: 60, type: 'furniture', color: '#8B4513' },
      { id: 'table', x: 400, y: 300, width: 100, height: 80, type: 'furniture', color: '#654321' },
      { id: 'bed', x: width - 250, y: 150, width: 150, height: 100, type: 'furniture', color: '#A0522D' },
      { id: 'counter', x: 250, y: height - 150, width: 200, height: 50, type: 'furniture', color: '#696969' },
      { id: 'bookshelf', x: 50, y: height - 200, width: 80, height: 120, type: 'furniture', color: '#8B4513' }
    );
  } else if (mapId === 'map-2') {
    for (let i = 0; i < 6; i++) {
      objects.push({
    ...
    objects.push(
      { id: 'desk-1', x: 600, y: 200, width: 100, height: 60, type: 'furniture', color: '#654321' },
      { id: 'desk-2', x: 600, y: 400, width: 100, height: 60, type: 'furniture', color: '#654321' }
    );
  } else {
    objects.push(
      { id: 'lab-table-1', x: 150, y: 150, width: 180, height: 80, type: 'furniture', color: '#2F4F4F' },
      { id: 'lab-table-2', x: 500, y: 200, width: 180, height: 80, type: 'furniture', color: '#2F4F4F' },
      { id: 'equipment-1', x: 200, y: 400, width: 100, height: 100, type: 'furniture', color: '#778899' },
      { id: 'equipment-2', x: width - 250, y: 300, width: 120, height: 120, type: 'furniture', color: '#778899' },
      { id: 'cabinet', x: 50, y: height - 180, width: 100, height: 100, type: 'furniture', color: '#4A4A4A' }
    );
  }
...
function updateBotPlayer(bot: Player, gameState: GameState, deltaTime: number, selectedMap: any) {
  ...
  if (bot.isChaser) {
    ...
    if (chaser) {
      ...
      // Don't run from invisible chasers
      if (distance < 200 && !chaser.isInvisible) {
        const awayX = bot.x + (bot.x - chaser.x);
        const awayY = bot.y + (bot.y - chaser.y);
...
function moveTowards(player: Player, targetX: number, targetY: number, speed: number, gameState: GameState, selectedMap: any) {
  ...
  if (distance > 5 && !isNaN(distance)) {
    const newX = player.x + (dx / distance) * speed;
    const newY = player.y + (dy / distance) * speed;

    // Validate new position
    if (!isNaN(newX) && !isNaN(newY) && !checkCollision(newX, newY, PLAYER_SIZE, gameState.objects, selectedMap.width, selectedMap.height)) {
      player.x = newX;
      player.y = newY;
